class TweetsController < ApplicationController
  ...
  def get_tweet_ids(stock, max_results)
    url = URI("https://api.twitter.com/2/tweets/search/recent?query=#{stock.ticker}&max_results=#{max_results}&tweet.fields=public_metrics,lang,entities&expansions=author_id&user.fields=public_metrics&sort_order=relevancy")
    ...
    filtered_response_array = response_array.select { |e| (e["entities"]["cashtags"].nil? || e["entities"]["cashtags"].count <= 3) && e["lang"] == "en" }
